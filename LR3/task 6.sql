...
SELECT f.facility, CONCAT(m.surname, ' ', m.firstname) AS 'Members', 
    IF(b.memid = 0, f.guestcost, f.membercost) AS 'Cost'
FROM members m
    JOIN bookings b ON b.memid = m.memid
    JOIN facilities f ON b.facid = f.facid
WHERE DATE(starttime) = '2012.09.14' AND IF(b.memid = 0, f.guestcost, f.membercost) > 30
ORDER BY IF(b.memid = 0, f.guestcost, f.membercost), 'Cost' DESC;